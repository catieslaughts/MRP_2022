...
tperi_arr = np.asarray([0.,.25,.5,.75]) * u.year #[0.,.25,.5,.75]
e_arr     = np.asarray([0.,.25,.5,.75])
inc_arr   = np.asarray([0.001,0.5,1,1.5]) *np.pi * u.rad #23.4406
omega_arr = np.asarray([0.,0.5,1,1.5]) *np.pi * u.rad # 
anode_arr = np.asarray([0.,0.5,1,1.5]) *np.pi * u.rad #

delta_v_arr = [0.01,5]*u.km/u.s
theta_arr = np.asarray([0.,0.5,1,1.5])*np.pi*u.rad
phi_arr =  np.asarray([0.,0.5,1,1.5])*np.pi*u.rad
...
for idx in tqdm(range(tperi_flat.size)):
	...
	#simulate first kicked particle
	...
	#find kicked elements
	P_prime, f_prime, a_prime, e_prime, inc_prime, omega_prime, anode_prime = kick_kep_elements_copied(delta_v, theta, phi, P_i, f[0], a_i, e_i, inc_i, omega_i, anode_i, M, m)
	
	#calculate tperi
	tperi_prime = Tanom_to_tperi(f_prime, e_prime, P_prime)
	#simulate second kicked particle
	X, Y, Xs, Ys, Zs, Xv, Yv, Zv = kep3d(t_orbit,P_prime,tperi_prime,a_prime,e_prime,inc_prime, omega_prime, anode_prime)
...
#to save to file:
header = 'tperi (yr), e, inc (rad), omega (rad), anode (rad), theta (rad), phi (rad), delta_v (km/s), distance (AU), Worked?'
#formatstr = '%.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %s'

np.savetxt('gridtest_output2.txt', np.c_[tperi_flat.to(u.yr).value, e_flat, inc_flat.to(u.rad).value/np.pi, omega_flat.to(u.rad).value/np.pi, anode_flat.to(u.rad).value/np.pi, theta_flat.to(u.rad).value/np.pi, phi_flat.to(u.rad).value/np.pi, delta_v_flat.to(u.km/u.s).value, dist_flat.to(u.AU).value, worked_out], header = header, delimiter=', ', fmt='%s')
