...
def write_param_file(kick_vel:u.m/u.s = 1.*u.km/u.s, v_i:u.km/u.s = 6.*u.km/u.s, 
		...
		num_per_shell = 400, n_steps = 500, n_shells = 3, R_star:u.au = 1*u.Rsun, 
		paramfile = 'params.csv'):
		
	paramdata = np.asarray([kick_vel.to(u.km/u.s).value, v_i.to(u.km/u.s).value, t_firstpass.to(u.year).value, t_start.to(u.year).value, delta_t.to(u.year).value, e_i, omega_i.to(u.rad).value, anode_i.to(u.rad).value, M.to(u.kg).value, m.to(u.kg).value, num_per_shell, n_steps, n_shells, R_star.to(u.au).value])
	paramheader = 'kick_vel,v_i,t_firstpass,t_start,delta_t,e_i,omega_i,anode_i,M,m,num_per_shell,n_steps,n_shells,R_star\n(u.km/u.s),(u.km/u.s),(u.year),(u.year),(u.year), ,(u.rad),(u.rad),(u.kg),(u.kg), , , ,(u.au)'
	
	np.savetxt(paramfile, [paramdata], delimiter=',', header = paramheader)
	
def read_param_file(paramfile = 'params.csv'):
	kick_vel,v_i,t_firstpass,t_start,delta_t,e_i,omega_i,anode_i,M,m,num_per_shell,n_steps,n_shells,R_star = np.loadtxt(paramfile, delimiter = ',')
	...
	n_shells, num_per_shell, n_steps = int(n_shells), int(num_per_shell), int(n_steps)
	
	return kick_vel,v_i,t_firstpass,t_start,delta_t,e_i,omega_i,anode_i,M,m,num_per_shell,n_steps,n_shells,R_star
