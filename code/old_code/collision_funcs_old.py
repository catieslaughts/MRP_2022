@u.quantity_input
def kick_kep_elements_old(delta_v:u.m/u.s, theta_i:u.rad, phi_i:u.rad, P:u.year, f_i:u.rad, a_i:u.m, e_i, inc_i:u.deg, omega_bar_i:u.deg, anode_i:u.deg, M:u.kg, m:u.kg):
	'''
...
# 	print(phi/np.pi)
	...
	#delta_v_vec = delta_v*np.asarray([Sp*Ct,St*Sp,Cp])
...
#	h_i_vec = np.cross(R_vec, vel_i) #used two eqs here to test math was working out right, it is
	...
	P_prime = (P*P*a_i*a_i*a_i/(a_prime*a_prime*a_prime))**(1./2)
...
@u.quantity_input
def kick_kep_elements_from_paper(delta_v:u.m/u.s, theta_i:u.rad, phi_i:u.rad, P_i:u.year, f_i:u.rad, a_i:u.m, e_i, inc_i:u.deg, omega_bar_i:u.deg, anode_i:u.deg, M:u.kg, m:u.kg):
	'''
	calculates the resulting orbital elements of a secondary given a velocity impulse is applied. 
	Adapted from section 2 and appendix A of Jackson et al. 2014
	...
	inc_prime = np.arccos((1 + np.sqrt(1-e_i*e_i)*dvvk*st*spf/(1+e_i*cf))/np.sqrt(hprimh2))
	...
	P_prime = ((P_i*P_i)*(a_i*a_i*a_i)/(a_prime*a_prime*a_prime))**(1./2)
